---
title: How to use flon.wrap
link_text: How to use flon.wrap
---

# 1. Installing the flon.wrap contract

The flon.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `flon.wrap`.
...
The actual public key used is not important since that data is only encoded into the `flon::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `flon::newaccount` action. It should look like:
```sh
...
```json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "flon", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
### 1.1.2 Propose the transaction to create the flon.wrap account
...
### 1.1.3 Review and approve the transaction to create the flon.wrap account
...
### 1.1.4 Execute the transaction to create the flon.wrap account
...
## 1.2 Deploy the flon.wrap contract
...
### 1.2.2 Propose the transaction to deploy the flon.wrap contract
...
### 1.2.3 Review and approve the transaction to deploy the flon.wrap contract
...
### 1.2.4 Execute the transaction to create the flon.wrap account
...
saving abi to retrieved-flon.wrap.abi
```
```sh
sha256sum contracts/flon.wrap/flon.wrap.wasm
```
```console
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/flon.wrap/flon.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-flon.wrap.abi, can then be compared to the original ABI of the contract (contracts/flon.wrap/flon.wrap.abi) to ensure they are semantically the same.

# 2. Using the flon.wrap contract
