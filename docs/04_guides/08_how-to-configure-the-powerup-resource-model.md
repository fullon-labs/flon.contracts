...
1. Build [eos-system-contracts](https://github.com/fullon-labs/flon.contracts) with `powerup` code.
...
```sh
# call to `cfgpowerup`
focli push action flon cfgpowerup "[`cat ./config.json`]" -p flon
```

#### Check state

```sh
focli get table flon 0 powup.state
```
