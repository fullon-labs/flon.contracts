...
class eosio_token_tester : public eosio_system::base_system_tester {
public:

   eosio_token_tester() {
      produce_blocks( 2 );

      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "flon.token"_n } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "flon.token"_n, acc, "accounts"_n, account_name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "flon.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, eosio_token_tester ) try {
   ...
   memcpy((int64_t*)&max, &amount, sizeof(share_type)); // hack in an invalid amount
