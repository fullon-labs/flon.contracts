...
public:
   ...
   eosio_wrap_tester() {
      ...
      set_code( "flon.wrap"_n, contracts::wrap_wasm(), &system_private_key );
      set_abi( "flon.wrap"_n, contracts::wrap_abi().data(), &system_private_key );
...
BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, eosio_wrap_tester ) try {
   ...
   check_traces( trace, {
                           {{"receiver", "flon.wrap"_n}, {"act_name", "exec"_n}},
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester ) try {
   ...
   transaction_trace_ptr trace = push_action( "flon.msig"_n, "exec"_n, "alice"_n, mvo()
                                                ("proposer",      "carol")
                                                ("proposal_name", "first")
                                                ("executer",      "alice")
   );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, eosio_wrap_tester ) try {
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of flon.wrap::exec.
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, eosio_wrap_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of flon.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( "flon.msig"_n, "exec"_n, "alice"_n, mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
